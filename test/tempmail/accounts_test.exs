defmodule Tempmail.AccountsTest do
  use Tempmail.DataCase

  alias Tempmail.Accounts

  import Tempmail.AccountsFixtures
  alias Tempmail.Accounts.{User, UserToken}
  ...
  describe "get_or_register_oauth_user/1" do
    test "creates a confirmed user without a local password" do
      {:ok, user} =
        Accounts.get_or_register_oauth_user(%{
          email: unique_user_email(),
          name: "OAuth User",
      ...
      refute Accounts.get_user_by_email_and_password(user.email, "any password")
    end

    test "updates an existing user profile from oauth attrs" do
      user = user_fixture()
      ...
      assert updated_user.image == "https://example.com/new-avatar.png"
